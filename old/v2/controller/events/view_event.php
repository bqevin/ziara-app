...
?>
 ...
 while($row = mysql_fetch_array($get_info,MYSQL_ASSOC))
 {
	...
	{
	  echo" <form method='post'action='../controller/events/message_event.php'>
...
while($row=mysql_fetch_array($status,MYSQL_ASSOC))
{
  ...
  echo"<a href='../controller/sessions/comment_event_status_session.php?event_status_id=$event_status_id'>$number Comment</a>";
...
else
{
 ...
 echo"
  ...
  <font color='blue'>{$row['status']}</font><br>";
   ...
   echo"<a href='../controller/sessions/comment_event_status_session.php?event_status_id=$photo_status_id'>$numberz Comment</a>";

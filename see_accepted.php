...
$get_friends = "SELECT * FROM friendship WHERE guest_friend='$my_email'";
$retval = mysql_query($get_friends,$link);
if(!$retval)
{
...
while($row = mysql_fetch_array($retval,MYSQL_ASSOC))
{
  ...
  $get_name = "SELECT * FROM info WHERE email='$host_friend'";
  $results = mysql_query($get_name,$link);
  if(!$results)
  {
    die('Error on 57:'.mysql_error());
  }
  while($row = mysql_fetch_array($results,MYSQL_ASSOC))
  {
	...
	$lname = $row['lname'];
	$the_email = $row['email'];
	echo"<a href='view_profile.php?email=$the_email' style='color:lime;text-decoration:none;'>$fname $lname</a>accepted your <a href='Friends.php'style='color:lime;text-decoration:none;'>friend request</a>";
